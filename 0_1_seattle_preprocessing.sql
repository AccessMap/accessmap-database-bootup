...
CREATE TABLE streets AS
	...
	FROM
		v_streets;
CREATE INDEX streets_index ON streets USING gist(geom);
...
CREATE TABLE sidewalks AS
	SELECT
		compkey AS id,
		ST_LineMerge(ST_Transform(wkb_geometry, 2926)) AS geom,
		segkey
	FROM v_sidewalks;
CREATE INDEX sidewalks_index ON sidewalks USING gist(geom);
ALTER TABLE sidewalks ADD PRIMARY KEY (id);
...
CREATE TABLE intersections AS
	...
	GROUP BY geom;
CREATE INDEX intersections_index ON intersections USING gist(geom);
ALTER TABLE intersections ADD PRIMARY KEY (id);
