...
CREATE OR REPLACE FUNCTION is_point_in_range(dg_range double precision[], dg_point double precision)
RETURNS boolean AS
$$
BEGIN
...
DROP TABLE IF EXISTS build.inter_groups;

CREATE TABLE build.inter_groups AS
      SELECT t_ig.i_id,
        ...
        FROM (    SELECT ig.*,degree_diff
              ...
              INNER JOIN build.intersection_groups AS ig
                      ON ig.i_id = ti.id
                   WHERE is_point_in_range(degree_diff,ST_Azimuth(ig.i_geom, ig.e_geom)) IS TRUE) AS t_ig
  INNER JOIN build.sidewalk_ends AS e
          ON t_ig.e_id = e.id
       WHERE is_point_in_range(t_ig.degree_diff,ST_Azimuth(t_ig.i_geom, ST_Centroid(e.geom)))
    GROUP BY t_ig.i_id
      HAVING count(t_ig.e_id) = 2;

DROP TABLE IF EXISTS build.t_gap_flags;

CREATE TABLE build.t_gap_flags AS
      SELECT t_ig.i_id,
             array_agg(e.sw_id) AS s_id,
             array_agg(e.sw_type) AS s_type,
             array_agg(e.geom) AS c_geom
