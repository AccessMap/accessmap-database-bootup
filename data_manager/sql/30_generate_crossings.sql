...
DROP TABLE IF EXISTS build.intersection_group;

CREATE TABLE build.intersection_group AS
      SELECT *
...
UPDATE build.intersection_group
  ...
  FROM (SELECT DISTINCT ON (q.c_id) q.c_id AS c_id,
                                    ...
                                    i.geom AS i_geom
                      FROM (SELECT t1.*
                              FROM build.intersection_group t1,
                                   build.intersection_group t2
 ...
 WHERE result.c_id = build.intersection_group.c_id;

DELETE FROM build.intersection_group
      WHERE i_id IS NULL;
...
IF pointd > degree[array_length(degree,1)]
THEN
    RETURN 1;
ELSE
    RETURN -1;
...
ALTER TABLE build.intersection_group
...
DROP TABLE IF EXISTS build.corner_group;

CREATE TABLE build.corner_group AS
      SELECT *
...
INSERT INTO crossings (geom, c1_id, c2_id)
     SELECT ST_MakeLine(q1.c_geom, q2.c_geom) as geom,
       ...
       FROM (  SELECT corner_group.*,
                 ...
                 FROM build.corner_group
                 JOIN build.intersections
                   ON build.intersections.id = corner_group.i_id
                WHERE build.intersections.is_t IS FALSE
                  AND num_s > 3) AS q1,
            (  SELECT corner_group.*,
                      intersections.degree_diff
                 FROM build.corner_group
      ...
      WHERE num.i_id = q1.i_id
        AND q1.i_id = q2.i_id
        AND (q1.range_group + 1 = q2.range_group -- clockwise
             OR
...
INSERT INTO crossings (geom, c1_id, c2_id)
     SELECT ST_ShortestLine(q1.s_geom, q2.c_geom) as geom,
      ...
      WHERE num.i_id = q1.i_id
        ...
        AND (q1.range_group + 1 = q2.range_group
             OR
             q1.range_group - 1 = q2.range_group
             OR
             q1.range_group/num.count = q2.range_group
             OR
             q2.range_group/num.count = q1.range_group);
